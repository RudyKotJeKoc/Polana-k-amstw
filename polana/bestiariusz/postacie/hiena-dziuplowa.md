# Hiena Dziuplowa (Aleksander Domek)

**Typ:** Zdrajca Systemu Prawnego / Adwokat Cyniczny
**Powiązania:** Wilk Samotnik (klient), Wyrok Karny II K 568/21, System prawny

## Opis

Hiena Dziuplowa (Aleksander Domek) to adwokat Dariusza (Wilka) w sprawie karnej II K 568/21, który dokonał jednego z najbardziej destrukcyjnych aktów zdrady w całej historii Polany Kłamstw. **5 minut przed rozprawą** (13.10.2021), na korytarzu sądowym, wykorzystał wyczerpanie psychiczne klienta i wymusił na nim **przyznanie się do winy** pod fałszywą obietnicą, że "sprawa będzie zamknięta, to tylko formalność".

Hiena symbolizuje **zdrajcę systemu prawnego** — adwokata, który powinien bronić klienta, ale zamiast tego stawia swój szybki zysk i wygodę ponad dobro osoby, której służył. Zimne oczy, fałszywy uśmiech, manipulacja na korytarzu — to obraz profesjonalisty, który sprzedał swojego klienta dla własnej korzyści.

Wyrok nakazowy, który powstał w wyniku tej zdrady, stał się **"koronnym dowodem"** w sprawie cywilnej I C 634/22. Jedna zdrada na korytarzu zniszczyła całą obronę Wilka.

## Symbolika

**Hiena jako padlinożerca**
Hieny żywią się padliną — Domek żywi się słabością klienta. Wykorzystuje wyczerpanie, desperację, zaufanie.

**Zdrada na korytarzu**
Najokrutniejszy rodzaj zdrady — tuż przed rozprawą, gdy klient jest najbardziej bezbronny. Nie w sali sądowej, nie w kancelarii — na korytarzu, w pośpiechu, bez świadków.

**Fałszywa obietnica**
*"Przyznaj się, a sprawa będzie zamknięta. To tylko formalność."* — klasyczna manipulacja. Obietnica bez pokrycia, obliczona na szybkie zakończenie sprawy.

**Zimne oczy, fałszywy uśmiech**
Symbol adwokata bez empatii, dla którego klient jest tylko źródłem dochodu, nie człowiekiem.

**Wyrok nakazowy jako broń**
Wynik zdrady Hieny — wyrok II K 568/21 (znęcanie psychiczne) — stał się "koronnym dowodem" w sprawie cywilnej. Jedna zdrada, dwie katastrofy.

## Rola w Historii

**W głównej baśni "Polana Kłamstw":**

Hiena Dziuplowa nie jest głównym antagonistą, ale jej **jeden akt zdrady** miał katastrofalne konsekwencje dla całej historii Wilka. Wyrok karny, wynikający z wymuszonego przyznania się, stał się fundamentem dla wyroku cywilnego o cofnięcie darowizny.

**Kluczowe wydarzenia:**

1. **13.10.2021, korytarz sądowy, 5 minut przed rozprawą**
   - Hiena Dziuplowa (Aleksander Domek) wykorzystuje stan psychiczny Wilka
   - Fałszywa obietnica: *"Przyznaj się, a sprawa będzie zamknięta. To tylko formalność."*
   - Wilk, wyczerpany, ulega
   - Wyrok nakazowy II K 568/21 — "znęcanie psychiczne"

2. **Konsekwencje zdrady**
   - Wyrok karny staje się "koronnym dowodem" w sprawie cywilnej I C 634/22
   - Obrona Wilka jest zniszczona — trudno obalić wyrok, na który sam się zgodził
   - Jedna zdrada na korytarzu = dwie przegrane sprawy

**W kronice:**
Hiena Dziuplowa reprezentuje **porażkę systemu obrony prawnej**. Adwokat, który powinien chronić klienta, zamiast tego go sprzedał. Jej rola to ostrzeżenie: **konflikt interesów, cyniczna manipulacja i brak etyki mogą zniszczyć życie klienta**.

## Cechy Charakterystyczne

- **Zdrajca systemu prawnego** — sprzedał klienta dla własnej wygody
- **Manipulator na korytarzu** — wykorzystał wyczerpanie psychiczne Wilka
- **Fałszywa obietnica** — *"To tylko formalność"*
- **Zimne oczy, fałszywy uśmiech** — brak empatii
- **Padlinożerca** — żywi się słabością klienta
- **Jeden akt, dwie katastrofy** — wyrok karny → "koronny dowód" w sprawie cywilnej
- **Symbol konfliktu interesów** — własna korzyść ponad dobrem klienta

## Tagi

`[zdrada, prawnik, manipulacja, konflikt interesów, wyrok karny, korupcja, cynizm]`

---

**Ostatnia aktualizacja:** 20 listopada 2025
**Status:** ✅ Wypełniony według szablonu FAZA 2
